...
import { FileText, Sparkles, Upload, X, CheckCircle } from "lucide-react";
...
export default function JobDescriptionInput({ value, onChange, disabled, interviewId }: JobDescriptionInputProps) {
  ...
  const handleFileUpload = async (file: File) => {
    if (!interviewId) {
      toast({
        ...
        description: "Please upload the interview audio before uploading a job description file.",
        variant: "destructive",
      });
      return;
    }
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: false,
    disabled: disabled || isUploadingFile || !interviewId
  });
      ...
      <CardContent className="p-6">
        ...
        <div
          ...
          className={cn(
            ...
            (disabled || isUploadingFile || !interviewId) && "pointer-events-none opacity-50"
          )}
